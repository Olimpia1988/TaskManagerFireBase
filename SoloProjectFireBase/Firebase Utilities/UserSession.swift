...
final class UserSession {
    weak var userSessionCreateAccountDelegate: UserSessionAccountCreationDelegate?
    ...
    public func createNewAccount(email: String, password: String) {
        Auth.auth().createUser(withEmail: email, password: password) { (authDataResult, error) in
            if let error = error {
                self.userSessionCreateAccountDelegate?.didRecieveErrorCreatingAccount(self, error: error)
            } else if let authDataResult = authDataResult {
                self.userSessionCreateAccountDelegate?.didCreateAccount(self, user: authDataResult.user)
